        ...
        {isLoading ? (
          ...
          <div className="flex flex-col gap-0 px-0 pb-24">
            {filtered.map((b: any) => (
              ...
              >
                {b.logo_url ? (
                  <img src={b.logo_url} alt="" className="w-14 h-14 rounded-xl object-cover shrink-0" />
                ) : (
                  <Monogram name={b.display_name || b.username} size={56} />
                )}
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <p className="font-bold text-[15px]" style={{ color: "#0A1628" }}>
                      {b.display_name || b.username}
                    </p>
                    {(b.is_verified || b.subscription_tier === "breeder_page") && (
                      <BadgeCheck className="w-4 h-4 shrink-0" style={{ color: "#C9A84C" }} />
                    )}
                  </div>
                  {b.farm_name && (
                    ...
                    <div className="flex gap-1.5 mt-1.5 flex-wrap">
                      {b.species_tags.map((s: string) => (
                        <span
                          ...
                          style={{ backgroundColor: "rgba(201,168,76,0.12)", color: "#8B6914", border: "1px solid rgba(201,168,76,0.25)" }}
                        >
                          {s}
                        </span>
                      ))}
                  ...
                  {b.bio && (
                    <p className="text-[12px] mt-1.5 line-clamp-2 leading-snug" style={{ color: "#9CA3AF" }}>
                      {b.bio}
                    </p>
                  )}
                ...
                <ChevronRight className="w-4 h-4 shrink-0 mt-1" style={{ color: "#D1D5DB" }} />
              </Link>
            ))}
          </div>
        ) : (
              ...
              >
                {b.hero_image_url || b.logo_url ? (
                  ...
                  <div className="w-full aspect-square flex items-center justify-center"
                    ...
                    <span className="text-2xl font-black" style={{ color: "rgba(201,168,76,0.4)" }}>
                      {(b.display_name || b.username || "?").charAt(0).toUpperCase()}
                    </span>
                  </div>
                )}
                <div className="p-2.5">
                  <p className="font-bold text-[13px] truncate" style={{ color: "#0A1628" }}>
                    {b.display_name || b.username}
                  </p>
                  {b.location && (
