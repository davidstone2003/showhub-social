...
export function BreederHero({ profile, isFullPage, stats }: BreederHeroProps) {
  const name = profile.display_name || profile.username;
  const hasSocials = profile.facebook_url || profile.instagram_url || profile.website_url;

  return (
    <div className="bg-card border-b border-border">
      {/* Hero image for full breeder page */}
      {isFullPage && profile.hero_image_url && (
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {profile.subscription_tier === "breeder_page" && (
            ...
            {profile.bio && (
              <p className={`text-sm text-muted-foreground mt-1.5 ${isFullPage ? "" : "line-clamp-2"}`}>
                {profile.bio}
              </p>
            )}
          </div>
        ...
        <div className="flex items-center gap-2 mt-4 flex-wrap">
          {profile.phone && (
            <Button variant="default" size="sm" asChild>
              <a href={`tel:${profile.phone}`}>Contact</a>
            </Button>
          )}
          {!profile.phone && (
